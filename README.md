...
The system automates the stock-analysis workflow:
'
             Stock Selection
                    │
                    ▼
...
The dashboard supports different historical periods:
...
The latest available trading-day data is retrieved when the dashboard is refreshed.
...
These features are used across the:
...
The models are evaluated against the same chronological forecasting setup.
...
This is important because complex neural networks should be compared against a simple baseline rather than being assumed to be better automatically.

🧠 LSTM Forecasting

The project uses a Long Short-Term Memory neural network for time-series forecasting.
...
The project also implements a Gated Recurrent Unit model.
...
The current evaluation results show that complex neural-network models do not consistently outperform the naive baseline across the evaluated stocks.
...
🏗️ System Architecture
      ...
      ML Classification  Forecasting   Risk Analysis
       ...
       │     │     │   │   │    │       │
       ▼     ▼     ▼   ▼   ▼    ▼       ▼
    Logistic RF XGB Naive LSTM GRU  Volatility
                                      Drawdown
                                      Returns
             │             │             │
             └─────────────┼─────────────┘
...
The current repository contains trained models for:
...
This allows the dashboard to load the previously trained models without retraining them every time the application starts.
...
Contains the ML and deep-learning components.
...
Repository:
Akshaya-126/AI-Stock-Market-Analysis
...
git commit -m "Update stock analysis system"
...
https://github.com/Akshaya-126/AI-Stock-Market-Analysis
...
If they disagree, the dashboard reports the disagreement.
